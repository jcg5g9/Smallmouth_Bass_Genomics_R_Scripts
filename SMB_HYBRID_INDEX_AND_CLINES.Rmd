...
## Plotting 
```{r}
setkey(hyb_index_hsyc_canbar_white$hi,POPID)

abc <- plot_h(data=hyb_index_hsyc_canbar_white$hi[c("HSYC_CANBAR","WHITE","BAYOU")],#Subset of POPIDs#
test.subject=hyb_index_hsyc_canbar_white$test.subject,
...
setkey(hyb_index_hsyc_white$hi,POPID)

abc <- plot_h(data=hyb_index_hsyc_white$hi[c("HSYC","WHITE","ELK","BUFF","SPAV","BSUG")],#Subset of POPIDs#
test.subject=hyb_index_hsyc_white$test.subject,
mean.h.by="POPID",			#Calculate the mean hybrid index for each value of the "POPID" column#
sort.by=c("mean_h","POPID","h_posterior_mode"),	#Order test subjects along the x axis by the mean hybrid 
								#index calculated above and also by individual hybrid index#
col.group="POPID",
...
hsyc_white_hyb_index_data <- hsyc_white_hyb_index_data %>%
  ...
  group_by(Source, POPID) %>%
  arrange(beta_mean, .by_group = T)
...
ggplot(hsyc_white_hyb_index_data, aes(x = INDLABEL, y = beta_mean, fill = POPID)) + 
  ...
  scale_fill_manual(values = c("sienna4","deepskyblue","orchid1","deeppink2")) +
  scale_color_manual(values = c("sienna4","deepskyblue","orchid1","deeppink2")) +
