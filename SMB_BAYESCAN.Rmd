...
bayes_all_fst <- read_excel("../../raw_data/outlier_fst_data/bayescan_data/smb_all_bayes_noBFC10_results_taxon_fst.xlsx")
bayes_smb_fst <- read_excel("../../raw_data/outlier_fst_data/bayescan_data/smb_smb_bayes_noBFC10_results_pop_fst.xlsx") #just smallmouth bass samples
bayes_neosho_fst <- read_excel("../../raw_data/outlier_fst_data/bayescan_data/neo_bayes_noBFC10_results_fst.xlsx") #just neosho samples
bayes_northern_fst <- read_excel("../../raw_data/outlier_fst_data/bayescan_data/nor_bayes_results_fst.xlsx") #just northern samples
```
...
bayescan_all_fst_significant <- bayes_all_fst %>% 
      filter(logPO >= 1.5) #extract only SNPs with logPO greater than or equal to 1.5 (significant)

bayescan_all_fst_not_significant <- bayes_all_fst %>% 
      filter(logPO < 1.5) #extract only SNPs with logPO less than 1.5 (not significant)

dim(bayescan_all_fst_significant) #1 total significant SNP
dim(bayescan_all_fst_not_significant) #50,827 insignificant SNPs
...
sig_label_all = as.data.frame(c(rep("significant", times = 1))) #no significant snps
      colnames(sig_label_all) <- c("selection")
      
non_sig_label_all <- as.data.frame(c(rep("insignificant", times = 50827))) #all non-significant snps
...
bayescan_all_fst_ready <- rbind(bayescan_all_fst_significant, bayescan_all_fst_not_significant)
```
...
bayescan_smb_fst_significant <- bayes_smb_fst %>% 
      filter(logPO >= 1.5) #extract only SNPs with logPO greater than or equal to 1.5 (significant)

bayescan_smb_fst_not_significant <- bayes_smb_fst %>% 
      filter(logPO < 1.5) #extract only SNPs with logPO less than 1.5 (not significant)

dim(bayescan_smb_fst_significant) #766 total significant SNPs
dim(bayescan_smb_fst_not_significant) #50,062 insignificant SNPs

#create separate columns of metadata for both datasets created in the above step (significant and not significant), which will be later appended to each dataset
sig_label_smb = as.data.frame(c(rep("significant", times = 766))) #no significant snps
      colnames(sig_label_smb) <- c("selection")
      
non_sig_label_smb <- as.data.frame(c(rep("insignificant", times = 50062))) #all non-significant snps
      colnames(non_sig_label_smb) <- c("selection")

#append metadata to each respective dataset
bayescan_smb_fst_significant <- cbind(bayescan_smb_fst_significant, sig_label_smb)
bayescan_smb_fst_not_significant <- cbind(bayescan_smb_fst_not_significant, non_sig_label_smb)

#combine both significant and non-significant datasets into one dataset with all data, now appropriately labeled
bayescan_smb_fst_ready <- rbind(bayescan_smb_fst_significant, bayescan_smb_fst_not_significant)
```

## Get Significant and Insignificant SNPs for Neosho data only
```{r}

bayescan_neosho_fst_significant <- bayes_neosho_fst %>% 
      filter(logPO >= 1.5) #extract only SNPs with logPO greater than or equal to 1.5 (significant)

bayescan_neosho_fst_not_significant <- bayes_neosho_fst %>% 
      filter(logPO < 1.5) #extract only SNPs with logPO less than 1.5 (not significant)

dim(bayescan_neosho_fst_significant) #45 total significant SNPs
dim(bayescan_neosho_fst_not_significant) #50,783 insignificant SNPs

#create separate columns of metadata for both datasets created in the above step (significant and not significant), which will be later appended to each dataset
sig_label_neosho = as.data.frame(c(rep("significant", times = 45))) #there are 45 total significant SNPs within Neosho Smallmouth Bass
      colnames(sig_label_neosho) <- c("selection")
non_sig_label_neosho <- as.data.frame(c(rep("insignificant", times = 50783)))
      colnames(non_sig_label_neosho) <- c("selection")

#append metadata to each respective dataset
bayescan_neosho_fst_significant <- cbind(bayescan_neosho_fst_significant, sig_label_neosho)
bayescan_neosho_fst_not_significant <- cbind(bayescan_neosho_fst_not_significant, non_sig_label_neosho)

#combine both significant and non-significant datasets into one dataset with all data, now appropriately labeled
bayescan_neosho_fst_ready <- rbind(bayescan_neosho_fst_significant, bayescan_neosho_fst_not_significant)
```
...
dim(bayescan_northern_fst_significant) # zero significant snps
dim(bayescan_northern_fst_not_significant) #50,828 neutral snps


#create separate columns of metadata for both datasets created in the above step (significant and not significant), which will be later appended to each dataset
sig_label_northern = as.data.frame(c(rep("significant", times = 0))) #no significant snps
      colnames(sig_label_northern) <- c("selection")
non_sig_label_northern <- as.data.frame(c(rep("insignificant", times = 50828))) #all non-significant snps
...
#combine both significant and non-significant datasets into one dataset with all data, now appropriately labeled
bayescan_northern_fst_ready <- rbind(bayescan_northern_fst_significant, bayescan_northern_fst_not_significant)
```
...
###smallmouth bass samples
bayescan_smb_fst_1000s <- bayescan_smb_fst_ready %>% 
      filter(logPO >= 1000) #extract only snps with logPO greater than or equal to 1000
bayescan_smb_fst_not1000 <- bayescan_smb_fst_ready %>% 
      filter(logPO < 1000) #extract only snps with log10 less than 1000

###neosho samples
bayescan_neosho_fst_1000s <- bayescan_neosho_fst_ready %>% 
...
bayescan_neosho_fst_not1000 <- bayescan_neosho_fst_ready %>% 
      filter(logPO < 1000) #extract only snps with log10 less than 1000
...
#Plot All samples
all_bayes_plot <- ggplot(bayescan_all_fst_not1000, aes(x = logPO, y = fst, color = selection)) + #make plot from non-1000 logPO for all SMB 
      geom_point(show.legend = F) + #do not show legend on the plot itself
      geom_vline(xintercept = 1.5, linetype = "longdash") + #add a vertical line at the x value 1.5, and make it a dashed line
      theme_set(theme_cowplot(12)) + #set ggplot to cowplot style 12 (gets rid of grid lines)
      scale_color_manual(values = c("red","black")) + #set the color of points to black
...
smb_bayes_plot <- ggplot(bayescan_smb_fst_not1000, aes(x = logPO, y = fst, color = selection)) + #make plot from non-1000 logPO for all SMB 
      geom_point(show.legend = F) + #do not show legend on the plot itself
      geom_vline(xintercept = 1.5, linetype = "longdash") + #add a vertical line at the x value 1.5, and make it a dashed line
      theme_set(theme_cowplot(12)) + #set ggplot to cowplot style 12 (gets rid of grid lines)
      scale_color_manual(values = c("red","black")) + #set the color of points to black
...
neosho_bayes_plot <- ggplot(bayescan_neosho_fst_not1000, aes(x = logPO, y = fst, color = selection)) + #make plot from non-1000 logPO for neosho only
      geom_point(show.legend = F) + #do not show legend on the plot itself
      geom_vline(xintercept = 1.5, linetype = "longdash", color = "black") + #add a vertical line at the x value 1.5, and make it a dashed line
      theme_set(theme_cowplot(12)) + #set ggplot to cowplot style 12 (gets rid of grid lines)
      scale_color_manual(values = c("red","black")) + #set the color of non-significant points to black and significant points to red
      labs(x = "Log10(Posterior Odds)", y = "FST") + #re-name the labels for the x and y axis
      theme(axis.title = element_text(size = 15, color = "black")) + #make the axis titles size 15
      theme(axis.text = element_text(size = 15, color = "black")) + #make the axis text size 15
      theme(axis.line = element_line(color = "black")) +
...
northern_bayes_plot <- ggplot(bayescan_northern_fst_not1000, aes(x = logPO, y = fst, color = selection)) + 
      geom_point(show.legend = F) + #do not show legend on the plot itself
      geom_vline(xintercept = 1.5, linetype = "longdash", color = "black") + #add a vertical line at the x value 1.5, and make it a dashed line
      theme_set(theme_cowplot(12)) + #set ggplot to cowplot style 12 (gets rid of grid lines)
      scale_color_manual(values = c("black")) + #set the color of points to black
      labs(x = "Log10(Posterior Odds)", y = "FST") + #re-name the labels for the x and y axis
      theme(axis.title = element_text(size = 15, color = "black")) + #make the axis titles size 15
      theme(axis.text = element_text(size = 15, color = "black")) + #make the axis text size 15
...
all_pops <- as.vector(filtered_metadata_all[,-c(1:5)])
smb_pops <- as.vector(filtered_metadata_smb[,-c(1:4,6)])
neosho_pops <- as.vector(filtered_metadata_neosho[,-c(1:4,6)])
northern_pops <- as.vector(filtered_metadata_northern[,-c(1:4,6)])
```
...
neo_scree_plot <- plot(neosho_pca, option = "screeplot") #3 PCs
nor_scree_plot <- plot(northern_pca, option = "screeplot") #2 PCs
...
all_pca_ms_plot <- ggplot(all_pca_scores, aes(x = PC1, y = PC2)) + 
   geom_point(aes(fill = pop), color = "black", pch = 21, size = 4, show.legend = T) +
...
```{r}
#Merge datasets for outlier SNPs and neutral SNPs for BAYESCAN and PCADAPT
neosho_bayescan_pcadapt_outlier_merged <- merge(neosho_pcadapt_snps_significant, bayescan_neosho_fst_significant, by = "snp_id")
neosho_bayescan_pcadapt_neutral_merged <- merge(neosho_pcadapt_snps_not_significant, bayescan_neosho_fst_not_significant, by = "snp_id")

#########This is where PCAdapt and Bayescan overlap#############

# 29 SNPs (signficant in both pcadapt analysis and bayescan)
# 35,025 SNPs (non-significant in both pcadapt analysis and bayescan)
```
